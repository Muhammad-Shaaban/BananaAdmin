...
export class ProductsService {
  ...
  baseUrl = environment.url + 'admin/products/';
  singleProdURL =  environment.url + 'admin/product/single/';
  addProdURL =  environment.url + 'admin/product';
  updateProdURL =  environment.url + 'admin/product/edit';
  deleteProdURL =  environment.url + 'admin/product/delete';
...
//'?page=1&date=1&sold=1&filter[]=1&filter[]=2'
  ...
  getStatistecs(){
    return this.http.get( environment.url + 'admin/home', {headers: {Authorization: 'hh ' + localStorage.getItem('token')}});
  }
